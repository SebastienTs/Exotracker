...
def detect_spots_msdog(vw: Viewer, spot_rad=1.5, min_intensity=0.33) -> LayerDataTuple:
    ...
    if viewer_is_layer(vw, 'Channel1'):
        ...
        for t in trange(img.shape[0]):
          blobs = dog(img[t, :, :], min_sigma=1, max_sigma=2*np.ceil(spot_rad), threshold=min_intensity*1e-3)
          blobs_coords = np.c_[(t*np.ones(blobs.shape[0]), blobs[:,:-1])]
          stats = disk_int_stats(img,  blobs_coords, 2*spot_rad, False)
          blb_kept =  [np.insert(blob[:2], 0, t).astype(int) for i, blob in enumerate(blobs)
                       if blob[2] <= spot_rad*1.25 and stats['min'][i] >= 1]
        ...
        detect_spots_msdog.call_button.text = f'Detected Spots ({len(blb_lst)})'

        return ([blob[:3] for blob in blb_lst],
                {'name': 'Blobs', 'size': int(np.round(5*spot_rad+1)), 'border_color': 'gray', 'face_color': 'transparent',
...
@magicgui(call_button='Track',
          ...
          max_gaps={'widget_type': 'IntSlider', 'max': 21},
          max_mean_speed={'widget_type': 'FloatSlider', 'max': 1},
          min_duration={'widget_type': 'IntSlider', 'max': 50})
def track_spots_pytrack(vw: Viewer, search_range=1, max_gaps=9, max_mean_speed=0.5, min_duration=35) -> LayerDataTuple:
    ...
    if viewer_is_layer(vw, 'Blobs'):
        ...
        for id, df in trajectories.groupby('particle'):
            ...
            if (duration >= min_duration) and (length/duration <= max_mean_speed):
...
@magicgui(call_button='Filter Tracks',
          ...
          max_eccent={'widget_type': 'FloatSlider', 'max': 2},
          bypass={'widget_type': 'Checkbox'},
          chan2_ext={'widget_type': 'IntSlider', 'max': 15},
          chan2_delta={'widget_type': 'FloatSlider', 'max': 1})
def filter_tracks(vw: Viewer, min_start_frame=9, max_end_frame=9, min_neighbor_dist=5, min_contrast=0.15,
                  max_eccent=0.26, bypass=False, chan2_ext=3, chan2_delta=0.08) -> LayerDataTuple:
  ...
  if viewer_is_layer(vw, 'Tracks') and viewer_is_layer(vw, 'Blobs'):
      ...
      for i, df in trajectories.groupby('particle', as_index=False):
          ...
          diskin = disk_int_stats(img, np.column_stack((df['frame'], df['y'], df['x'])), int(np.round(0.75*spot_rad)), False)
          diskout = disk_int_stats(img, np.column_stack((df['frame'], df['y'], df['x'])), int(np.round(1.5*spot_rad)), True)
          ...
          if (first_frame >= min_start_frame and last_frame <= (img.shape[0]-max_end_frame) and dst_flags[cnt_tracks]
                  and contrast >= min_contrast and eccent <= max_eccent) or bypass:
      ...
      if load_image_tiff.loadchan2.value:
          ...
          border_colors = np.where(colors, color_codes[1], color_codes[0]).tolist()
      else:
          border_colors = 'green'
