...
def reanalyze_tracks(tracks_props, medrad, trck_thr, first_trck, last_trck):
    cnt = 1
...
def  plot_tracks_intensity(tracks_props, tracks_c2_times, first_trck, last_trck, medrad, int_norm, int_preframe, int_postframe):
    cnt = 1
    for key, value in list(tracks_props.items())[:]:
        if tracks_props[key]['ch2_positive'] == 1:
            if cnt >= first_trck and cnt <= last_trck:
...
def plot_tracks_avg_intensity(tracks_props, medrad, int_norm, proteins, int_preframe):
...
def graph_tracks(groupfiles=False, plot_intprofiles=False, plot_first_trck=1, plot_last_trck=25, plot_avg_intprofile=True, plot_timelines=True):

    # Close all plots
    plt.close('all')

    # Identify results files in image folder
    ...
    proteins = [0, 0]
    for key, value in list(tracks_props.items())[:]:
        frame_timestep = tracks_props[key]['frame_timestep']
        int_preframe = tracks_props[key]['int_preframe']
        int_postframe = tracks_props[key]['int_postframe']
        proteins[0] = tracks_props[key]['protein1']
        proteins[1] = tracks_props[key]['protein2']
        break
    print(f'Frame timestep: {frame_timestep} s')
    print(f'Number of preframe(s): {int_preframe}')
    print(f'Number of postframe(s): {int_postframe}')
    ...
    if reanalyze:
        reanalyze_tracks(tracks_props, medrad = 4, trck_thr = 0.35, first_trck=plot_first_trck, last_trck=plot_last_trck)
    if model:
        fit_plateaus(tracks_props, timestep=frame_timestep, first_trck=plot_first_trck, last_trck=plot_last_trck)
    if plot_intprofiles:
        plot_tracks_intensity(tracks_props, tracks_c2_times, first_trck=plot_first_trck, last_trck = plot_last_trck, medrad = 4, int_norm = False, int_preframe = int_preframe, int_postframe = int_postframe)
    if plot_avg_intprofile:
        plot_tracks_avg_intensity(tracks_props, medrad = 4, int_norm = True, proteins=proteins, int_preframe = int_preframe)
    if plot_timelines:
        plot_tracks_timelines(data_list, proteins=proteins, timestep=frame_timestep)
